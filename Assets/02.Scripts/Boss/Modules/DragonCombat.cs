...
public class DragonCombat
{
    ...
    public void DarkProjectileEffect()
    {
        ...
        var projectile = _controller.ObjectContainer.GetDirectionalPoolObject(DarkProjectileKey);
        ...
        projectile.Fire(_controller.transform.forward, param.Speed, param.LifeTime, param.Damage,
            () => _controller.ObjectContainer.TakeDirectionalPool(DarkProjectileKey, projectile));
    }
    ...
    public void WindStormEffect()
    {
        ...
        var projectile = _controller.ObjectContainer.GetDirectionalPoolObject(WindStormKey);

        projectile.transform.position = spawnPoint;
        var param = _controller.ParamLoader.RightScratch_Special;
        projectile.Fire(direction, param.Speed, param.LifeTime, param.Damage,
            () => _controller.ObjectContainer.TakeDirectionalPool(WindStormKey, projectile));
    ...
    {
        var vfx = _controller.ObjectContainer.BreathParticlePool.Get();
        ...
        vfx.Init(duration, _controller.HasStateAuthority, damage, _controller.transform,
            () => _controller.ObjectContainer.BreathParticlePool.Take(vfx));
    }
        ...
        float damage, float floorDamage)
    {
        ...
        if (_controller.HasStateAuthority)
        {
            ...
            var proj = _controller.Runner.Spawn(
                _controller.ObjectContainer.LavaProjectile, // NetworkObject 붙은 프리팹
                spawnPoint,
                Quaternion.identity,
            ...
            Action onArrived = () =>
            {
                ...
                _controller.Runner.Spawn(
                    _controller.ObjectContainer.LavaFloorPrefab, targetPos, Quaternion.identity,
                    onBeforeSpawned: (runner, obj) =>
                    {
    ...
    public void PerformRoarAttack(float radius, int count, float duration, float floorDamage)
    {
        ...
        // 공격 실행
        _controller.ObjectContainer.transform.position = _controller.transform.position;
        _controller.ObjectContainer.RoarExplosion.Reset(radius, count, interval, floorDamage);
    ...
    public void PerformBloodExplode(float duration, float targetSize, float remainDuration, float damage)
    {
        ...
        if (_controller.HasStateAuthority)
        {
            var explosion = _controller.Runner.Spawn(_controller.ObjectContainer.BloodExplosionPrefab, _controller.transform.position,
                Quaternion.identity,
                onBeforeSpawned: (runner, obj) =>
                {
